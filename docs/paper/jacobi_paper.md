...
**Mathematical meaning:** The Jacobi set $\mathbb{J}(f_1,\ldots,f_k)$ of $k \leq d$ Morse functions on a $d$-manifold $\mathbb{M}$ is defined as the closure of the set of simultaneous critical points — points where the $k$ gradients become linearly dependent. For $k=2$ on a 2-manifold, $\mathbb{J}$ is generically a union of disjoint smooth closed curves. For general $k$, $\mathbb{J}$ is a $(k-1)$-dimensional submanifold as long as $d \geq 2k-3$ (⚠️ 原文条件为 $d \geq 2k-1$，经 Boardman 分层余维数分析修正为 $d \geq 2k-3$，详见 Section 5 纠正说明).
...
In the simplest non-trivial case, we have two Morse functions on a common 2-manifold. In this case, the Jacobi set $\mathbb{J} = \mathbb{J}(f, g)$ is generically a collection of pairwise disjoint smooth curves that are free of any self-intersections. Figure 1 illustrates the concept for two Morse functions on the two-dimensional torus. The Jacobi set of a generic collection of $k$ Morse functions is a submanifold of dimension $k-1$, provided $d \geq 2k-3$. The first time this inequality fails — meaning $\Sigma_2$ first becomes non-empty — is for $d = 2k-4$; under the paper's scope $k \leq d$, the smallest such case is $k = 4$, $d = 4$, where the Jacobi set is a 3-manifold except at a discrete set of points.*(原文条件为 $d \geq 2k-1$ 且首次失效为 $d = 2(k-1)$，经 Boardman 分层余维数分析修正，详见 Section 5.)*
...
**Table interpretation:** This table classifies vertices based on the reduced Betti numbers of their lower links. Each row corresponds to a vertex type (regular, or critical with a given Morse index). Each column $\tilde{\beta}_p$ is the $p$-th reduced Betti number. A vertex is regular iff its lower link is non-empty and contractible (all $\tilde{\beta}_p = 0$). A vertex is a simple critical point of index $\lambda$ iff its lower link has the homology of a $(\lambda-1)$-sphere — for $\lambda = 0$, this is $\mathbb{S}^{-1}$ (the empty set), giving $\tilde{\beta}_{-1} = 1$; for $\lambda \geq 1$, this is a $(\lambda-1)$-sphere, giving exactly one $\tilde{\beta}_{\lambda-1} = 1$ (all other $\tilde{\beta}_p = 0$). For example, a minimum (index 0) has $\tilde{\beta}_{-1} = 1$ (empty lower link); a 1-saddle has $\tilde{\beta}_0 = 1$ ($\mathbb{S}^0$, i.e., two disconnected points); a maximum in a 3-manifold (index 3) has $\tilde{\beta}_2 = 1$.

|                     | $\tilde{\beta}_{-1}$ | $\tilde{\beta}_0$ | $\tilde{\beta}_1$ | $\tilde{\beta}_2$ | $\tilde{\beta}_3$ |
|---------------------|-----------------------|-------------------|-------------------|-------------------|-------------------|
| regular             | 0                     | 0                 | 0                 | 0                 | 0                 |
...
> **Table 1:** Classification of vertices into regular points and simple critical points (identified by their index) using the reduced Betti numbers of the lower link. *(原文表 1 存在指标偏移——指标 $\lambda$ 错标为 $\tilde{\beta}_\lambda=1$，此处已按 $\tilde{\beta}_{\lambda-1}=1$ 修正，与正文文字描述一致。若与原文对照，请注意此差异。)*
...
**Algorithmic intent:** Given a simplicial complex $L$ (the lower link of a vertex or simplex), compute its reduced Betti numbers $\tilde{\beta}_p$ for all dimensions $p \geq -1$. Return their sum, which is the multiplicity $\mu$ — zero means regular, one means simple critical, two or more means multiple critical points have coalesced. The inclusion of $\tilde{\beta}_{-1}$ is essential: for a minimum, the lower link is empty, giving $\tilde{\beta}_{-1}=1$ and all other $\tilde{\beta}_p = 0$; without $\tilde{\beta}_{-1}$, a minimum would be misclassified as regular.

```
integer ISCRITICAL (Complex L)
  foreach p ≥ -1 do compute β̃_p of L endfor;
  return Σ_{p≥-1} β̃_p.
```

where:
- `β̃_p` — the $p$-th reduced Betti number $\tilde{\beta}_p$ of the complex $L$ (computed over $\mathbb{Z}_2$ coefficients)
- The sum $\sum_{p \geq -1} \tilde{\beta}_p$ is the total multiplicity $\mu$ of the critical point (原文伪代码为 `Σ_{p≥0}`，无法检测极小值——因极小值下链为空集，仅有 $\tilde{\beta}_{-1}=1$。此处已修正求和范围。)
