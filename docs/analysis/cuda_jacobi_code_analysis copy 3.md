...
因此，本报告分析的重点不是简单说明“使用了 CUDA”，而是说明为什么该算法能够按边并行、如何将串行数据结构转换为 GPU 可处理的数组、怎样在保持论文判定语义的同时处理浮点退化问题，以及如何通过比对和可视化验证并行结果的正确性。
...
这也是本项目区别于简单 CUDA 改写的地方：如果只把非 SoS 分支搬到 GPU，虽然可以得到并行代码，但在退化数据上可能与串行参考不一致；如果完整移植 SoS 到 GPU，工作量又过大。本项目采用混合回退，使结果能够和串行 SoS 输出完全一致。
...
该测试说明 benchmark 脚本可以正常生成网格、运行串行与并行程序、完成正确性比对并输出 CSV。`match=yes` 表示该规模下 GPU 输出与串行输出一致。
...
可视化验证进一步说明，CUDA hybrid 程序输出的 Jacobi 集不仅在数值比对中与串行结果一致，而且在几何形态上也符合预期。对于 `f=y`、`g=x` 的 torus 示例，Jacobi 集表现为 `z≈0` 截面上的两条闭合圆曲线，叠加到 torus 曲面后对应圆环的内外两圈。该结果与前面文本比对和性能测试共同验证了并行实现的正确性。

## 实验总结与心得

本次实验从论文理解、串行代码阅读到 CUDA 并行化实现，完整经历了一个算法工程化的过程。最初阅读论文时，Jacobi 集更多是一个偏理论的概念，涉及 Morse 函数、流形、临界关系和 lower link 等内容；而串行项目将这些理论转化为三角网格上的具体边判定，使抽象算法有了可以调试和验证的程序形式。通过分析 `JacobiSet::compute()`、`POS1`、`POS2` 和 `is_lowerLink()`，可以看到论文中的局部判定思想最终落到了每条内部边及其 link 顶点上。正是这种局部性，使得 CUDA 并行化有了明确的切入点：每条边的判断相互独立，可以自然映射为一个 GPU 线程。这个过程让我认识到，并行化并不是简单地把代码放进 `.cu` 文件，而是要先找到算法中真正独立、重复、规模较大的计算单元。

在实现过程中，最大的体会是正确性比单纯追求加速更重要。Jacobi 集计算中存在浮点退化问题，串行项目使用 SoS 保证符号判定稳定。如果完全忽略这一点，只实现普通 double 精度 GPU 判定，虽然代码会更简单，但结果可能在退化边上与串行参考不一致。因此本实验采用 CPU-GPU 混合方案：CPU 负责网格读取、拓扑预处理和 SoS 回退，GPU 负责大规模普通边的并行判定。测试结果也说明，小规模下端到端性能会受到启动、传输、预处理和 SoS 初始化开销影响，但随着网格规模增大，GPU 并行判定的优势逐渐体现。最终程序能够在保持与串行结果一致的前提下，在大规模网格上获得一定加速，这比单纯展示一个不可验证的 CUDA 加速结果更有意义。

通过本次实验，我也更加理解了异构并行程序的设计取舍。GPU 擅长处理结构规则的大量数值计算，但并不适合直接承载所有复杂控制逻辑和动态数据结构。合理的做法是把 CPU 和 GPU 各自擅长的部分组合起来，并通过清晰的数据接口连接。本项目中的 `EdgeRecord`、`JacobiGpuResult`、比对脚本和 benchmark 脚本，都是为了让并行实现既可运行、可验证，也便于后续扩展。后续如果继续优化，可以考虑减少 CPU 预处理开销、改进 GPU 数据布局、支持外部标量场输入，并进一步研究 SoS 判定在 GPU 上的实现可能性。
